...
class OnRawReaction(commands.Cog, name='On Raw Reaction Listeners and Events'):
    ...
    async def on_raw_reaction_add(self, payload:discord.RawReactionActionEvent):
        ...
        if emoji == '⛔' and payload.message_id == payload.user_id:
            member_name = self.client.get_user(int(payload.user_id))
            await member.send('test report')
            await payload.remove(payload.user_id)
        elif emoji == '🎟' and payload.message_id == int(message_id_db):
            member_name = self.client.get_user(payload.user_id)
            await member_name.send('test ticket')
            await payload.message_id.payload.remove(payload.user_id)
